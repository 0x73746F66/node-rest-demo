...
function handleGet(connection,req,res) {
    ...
    connection.query('SELECT * FROM category ORDER BY id DESC LIMIT ' + limit, function handleSql(err, rows) {
        if (err){ logAndRespond(err,res); return; }
...
function handleFind(connection,req,res) {
    var find = function find(id){
        connection.query('SELECT * FROM category WHERE id = ?', id, function handleSql(err, rows) {
            if (err){ logAndRespond(err,res); return; }
            if (rows.length === 0){ res.send(204); return; }
            res.send({
                result: 'success',
...
function handleIns(connection,req,res) {
    connection.query('INSERT INTO category SET ?', req.body, function handleSql(err, result) {
        if (err){ logAndRespond(err,res); return; }
        res.statusCode = 201;
        res.send({
            result: 'success',
...
function handleUpd(connection,req,res) {
    connection.query('UPDATE category SET ? WHERE id='+req.params.id, req.query, function handleSql(err) {
        if (err){ logAndRespond(err,res); return; }
        handleFind(connection,req,res)
    });
}
function handleDel(connection,req,res) {
    connection.query('DELETE FROM category WHERE id = ?', req.params.id, function handleSql(err) {
        if (err){ logAndRespond(err,res); return; }
        res.send({
            result: 'success',
            err:    '',
